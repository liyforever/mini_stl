...
private:
  typedef _MY_STL::hashtable<_MY_STL::pair<const Key, Type>, Key, Hash,
                             _MY_STL::select1st<_MY_STL::pair<const Key, Type> >,
                            EqualKey, Alloc>
    HT_type;
  HT_type Myc_;
public:
...
public:
  ...
  {
    MINI_STL_DEBUG_POINTER(_Hfn, "unordered_map Invalid hasher");
    MINI_STL_DEBUG_POINTER(_Comp, "unordered_map Invalid EqualKey");
  }
  ...
  {
    MINI_STL_DEBUG_RANGE_OF_ITERATOR(_First, _Last, "unordered_map unordered_map()");
    MINI_STL_DEBUG_POINTER(_Hfn, "unordered_map Invalid hasher");
    MINI_STL_DEBUG_POINTER(_Comp, "unordered_map Invalid EqualKey");
...
public:
  ...
  {
    return Myc_.bucket_size(nbucket);
  }
  ...
  {
    MINI_STL_DEBUG_RANGE_OF_ITERATOR(_First, _Last, "unordered_map erase");
  ...
  {
    MINI_STL_DEBUG_RANGE_OF_ITERATOR(_First, _Last, "unordered_map insert");
    Myc_.insert_unique(_First, _Last);
  }
  ...
  bool operator==(const unordered_map& _Right)
  {
...
template<class Key,
    ...
    class EqualKey,
    class Alloc>
void swap(unordered_map<Key,Type,Hash,EqualKey,Alloc>& _Left,
          unordered_map<Key,Type,Hash,EqualKey,Alloc>& _Right)
{
  _Left.swap(_Right);
}
MINI_STL_END
